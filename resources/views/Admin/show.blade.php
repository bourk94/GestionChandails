                        ...
                        @foreach ($articles->unique('article_id') as $article)
                            <div class="w3-col l3 m6 w3-margin-bottom">
                                <div class="w3-panel w3-border w3-round-large">
                                    <br>
                                    <img src="{{ asset('/img/' . $article->type . '/' . $article->type . '.jpg') }}"
                                        alt="{{ $article->type }}" style="width:100%" />
                                    <h3>{{ $article->nom }}</h3>
                                        ...
                                        @foreach ($tailles->where('article_id', 'like', $article->article_id) as $taille)
                                            <label>
                                                ...
                                                <div class="button"><span
                                                        class="w3-text-black">{{ $taille->format }}</span>
                                                </div>
                                            </label>
                                        @endforeach
                                    </div>

                                    <a href="{{ route('articles.editArticleCampagne', [$article->article_campagne_id]) }}"
                                        class="a_decoration_none">
                                        <button class="w3-button w3-block w3-hover-red btnColor"
