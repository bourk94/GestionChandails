...
@section('contenu')
    ...
    @if (count($campagnes->where('statut', 'en cours')) > 0)
        ...
        <div class="w3-content" style="max-width:1100px;margin-top:80px;margin-bottom:80px">
            ...
            <h1 class="center">Modifier la campagne {{$campagnes->where('statut', 'en cours')->first()->nom_campagne}}</h1>
        
            <form class="w3-container" action="{{ route('campagnes.update', $campagnes->where('statut', 'en cours')->first()->id) }}" method="POST">
            @csrf
            @method('PATCH')
        
                <div class="w3-section">
                    <label for="nom">Nouveau nom de la campagne :</label>
                    <input class="w3-input w3-border w3-hover-border-black" style="width:100%;" type="text" name="nom_campagne" id="nom_campagne" value="{{ old('nom_campagne', $campagnes->where('statut', 'en cours')->first()->nom_campagne)}}">
                </div>
                <div class="w3-section">
                    <label for="dateDebut">Nouvelle date de début :</label>
                    <input class="w3-input w3-border w3-hover-border-black" style="width:100%;" type="date" name="date_debut_campagne" id="dateDebut" value="{{ old('date_debut_campagne', $campagnes->where('statut', 'en cours')->first()->date_debut_campagne)}}">
                </div>
                <div class="w3-section">
                    <label for="dateFin">Nouvelle date de fin :</label>
                    <input class="w3-input w3-border w3-hover-border-black" style="width:100%;" type="date" name="date_fin_campagne" id="dateFin" value="{{ old('date_fin_campagne', $campagnes->where('statut', 'en cours')->first()->date_fin_campagne)}}">
                </div>
                <div class="w3-section">
                    <label for="dateDebutCollecte">Nouvelle date de début de la collecte :</label>
                    <input class="w3-input w3-border w3-hover-border-black" style="width:100%;" type="date" name="date_debut_collecte" id="date_debut_collecte" value="{{ old('date_debut_collecte', $campagnes->where('statut', 'en cours')->first()->date_debut_collecte)}}">
                </div>
                <div class="w3-section">
                    <label for="dateFinCollecte">Nouvelle date de fin de la collecte :</label>
                    <input class="w3-input w3-border w3-hover-border-black" style="width:100%;" type="date" name="date_fin_collecte" id="date_fin_collecte" value="{{ old('date_fin_collecte', $campagnes->where('statut', 'en cours')->first()->date_fin_collecte)}}">
                </div>
                <div class="w3-section">
